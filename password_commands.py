...
from passwords_helper import check_pass_complexity, encrypt_password, decrypt_password
...
def change_pass(ctx, name):
    ...
    print(f"Password {name} has been deleted")
    return name
...
def delete_pass(ctx, name, username, password):
    ...
    print(f"Password with name {name} has been updated.")
    return name
